<div class="container mx-auto">
    ...
    <div class="bg-white rounded-2xl shadow-lg max-w-xs sm:max-w-md lg:max-w-lg w-full overflow-y-auto" style="max-height: 80vh;">
        <div class="sticky top-0 bg-gray-100 p-2 flex justify-between items-center rounded-t-2xl z-10">
            ...
            <h1 class="text-xl font-extrabold text-gray-900">{{ $selectedPokemon->name }}</h1>
            <span class="text-gray-500 text-sm p-3">#{{ sprintf('%03d', $selectedPokemon->id) }}</span>
        </div>

        <div class="flex justify-center mt-2">
            <img src="{{ asset('storage/img/pokemons/' . $selectedPokemon->image) }}" alt="{{ $selectedPokemon->name }}" class="w-40 h-40 object-contain rounded-xl shadow-lg transition-transform transform hover:scale-105">
        </div>
        ...
        <div class="p-4">
            ...
            <div class="flex justify-center space-x-2 mb-4">
                <div class="w-1/2 p-2 bg-gray-50 rounded-lg text-center">
            ...
            <ul class="space-y-2">
                @foreach($selectedPokemon->attacks as $attack)
                    <li class="bg-gray-100 rounded-xl p-3 hover:bg-gray-200 transition duration-300 ease-in-out">
