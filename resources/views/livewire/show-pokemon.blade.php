<div class="max-w-lg mx-auto bg-white rounded-3xl shadow-lg overflow-hidden transform transition duration-500 hover:shadow-2xl">
    ...
    <div class="p-8">
        <div class="flex justify-center space-x-2 mb-6">
            <span class="inline-block px-4 py-1 rounded-full text-sm font-semibold text-white flex items-center" style="background-color: {{ $pokemon->type1->color }}">
                <img src="{{ $pokemon->type1->image }}" alt="{{ $pokemon->type1->name }}" class="w-8 h-8 mr-2">
                {{ $pokemon->type1->name }}
            </span>
            @if($pokemon->type2)
                <span class="inline-block px-4 py-1 rounded-full text-sm font-semibold text-white flex items-center" style="background-color: {{ $pokemon->type2->color }}">
                    <img src="{{ $pokemon->type2->image }}" alt="{{ $pokemon->type2->name }}" class="w-8 h-8 mr-2">
                    {{ $pokemon->type2->name }}
                </span>
            @endif
        </div>
